...
float Threshold1(GENDRIVE *Carrier, float RZBit1Gauss[], float *OSigmaNoise, float *DSigmaNoise, DeviceSTR *Network, unsigned int transmitters, SuperGaussian *gauss, float IthFrac) { 
  ...
  unsigned int Min= SP- Range/2,Max= SP+ Range/2;
  float n= Max- Min+1;
  ...
  for(Pul=0; Pul<NPulThr; Pul++) {
    ...
    for(i=0; i<(*Carrier).NPtos_Bit; i++) {
      ...
      One[i]*= RZBit1Gauss[i];
      One[i]+= (float) transmitters* (*gauss).P_0;
      One[i]= sqrt(One[i] );
    ...
    /*
    // Sampling TEST
    float OneS[Range], ZeroS[Range];
    for(i=Min; i<Max+1; i++) {
      ZeroS[i-Min]= Zero[i];
      OneS[i-Min]= One[i];
    }
    Write2Col("Samp.dat", ZeroS, OneS, Range+1, 0, Carrier);	// TEST one & zero to file
    */
    ...
    */
    ...
    // Idp= (sigma0* I1+ sigma1* I0)/ (sigma0+ sigma1);  // TEST Ith comment
    Idp= I0+ (I1- I0 )* IthFrac;	// TEST Ith uncomment
    // printf("IthFrac= %e\n",IthFrac); // TEST Ith
    // printf("Idp= %e\n",Idp); // TEST
    Qdp= (I1- I0)/ (sigma0+ sigma1);	// TEST QBER 
  ...
  // Id*= 1/ (float) NPulThr;
  // printf("Id= %e\n",Id); // TEST
  Qd*= (1/ (float) (NPulThr- reject) );	// TEST QBER
  // printf("%e\n",Qd); // TEST QBER UNCOMMENT
  return Id;
}


float BitSlots(GENDRIVE *Carrier, SuperGaussian *gauss, DeviceSTR *Network, float OSNR, float *OSigmaNoise, float DSNR, float *DSigmaNoise, unsigned int transmitters, float RZBit1Gauss[], float AuxTemp[], float IthFrac) {	
  unsigned long int i;
  // Petit AuxTemp
  ...
  *DSigmaNoise= Pmean_Rxin* (*Network).R* sqrt(1/ DSNR);	// amplitude of noise (Agrawal 4.4.11) [Santiago I.]
  ...
  // DETECTOR THRESHOLD
  float Id= Threshold1(Carrier, RZBit1Gauss, OSigmaNoise, DSigmaNoise, Network, transmitters, gauss, IthFrac);
  return Id;
}
...
*/
  ...
  float Id= BitSlots(&Carrier, &gauss, &Network, OSNR, &OSigmaNoise, DSNR, &DSigmaNoise, transmitters, RZBit1Gauss, AuxTemp, IthFrac);
