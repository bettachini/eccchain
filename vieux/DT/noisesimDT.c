  ...
  */
  ...
  switch ((*Network).Efilt_CutOff ) { 
    ...
    case 14:
      ...
      *SamplingPoint= 50;	// opt: 17.5 ord 6 / elec: 14 ord 2
    break;
    default:
      eGAIN= 2.326204969e+02; 	// CutOff= 14 GHz
      *SamplingPoint= 50;	// opt: 17.5 ord 6 / elec: 14 ord 2
...
float Threshold3(GENDRIVE *Carrier, float RZBit1Gauss[], float *OSigmaNoise, float *DSigmaNoise, DeviceSTR *Network, unsigned int transmitters, SuperGaussian *gauss) {
  ...
  Range++;			// for correct sampling size
  // printf("%i\n",SP); // TEST

  // Sampling (mean at each bit slot)
  ...
  for(Pul= 0; Pul< NPulThr; Pul++) {
    ...
    if (RanStr[Pul]==0) {
      Samp0[pos0]= AuxSum/ (float) Range;
      // printf("0m %e\n",Samp0[pos0]);		// TEST
      pos0++;
    }
    else {
      Samp1[pos1]= AuxSum/ (float) Range;
      // printf("1m %e\n",Samp1[pos1]);		// TEST
      pos1++;
    }
  }
