...
void InputParameters(GENDRIVE *Carrier, SuperGaussian *gauss, DeviceSTR *Network) {
  ...
  (*Network).Ofilt_CutOff= 25;			// [GHz] cut-off freq optical filter
...
// float BitSlots(GENDRIVE *Carrier, SuperGaussian *gauss, DeviceSTR *Network, float OSNR, float *OSigmaNoise, float DSNR, float *DSigmaNoise, unsigned int transmitters, float RZBit1Gauss[], float AuxTemp[], float IthFrac) {		// TEST Ith uncomment
  ...
  */
  ...
  Pmean_EDFAout*= (*Network).FAtt1;
  if ((*Banderas).Pmeans ) printf("Pmean_EDFAin[W]= %3e\n",Pmean_EDFAout);		// TEST PMEAN
  Pmean_EDFAout*= (*Network).EDFAgain;
  Pmean_EDFAout*= (*Network).EDFAgain;
  if ((*Banderas).Pmeans ) printf("Pmean_EDFAoutWON[W]= %3e\n",Pmean_EDFAout);		// TEST PMEAN
...
int main(int argc, char **argv) {
  ...
  // Flags
  int c;
  unsigned int transmitters= 128;					// Argument: number of Tx
  float DSNR, OSNR;	// Argument: SNR [dB] -> SNR [1]
  FLAGS Banderas; 
  ...
  // Using getopt
  while ((c= getopt(argc, argv, "elpd:o:") ) != -1)
    switch (c) {
      case 'e':
  ...
  while (--argc> 0 && (*++argv)[0] == '-') {
    while (c= *(++argv[0])) {
      ...
      case 'p':
        ...
        break;
      case 'n'
        DSNR= pow(10.0,atof(argv[2])/10.0), OSNR= pow(10.0,atof(argv[3])/10.0); // Argument: SNR [dB] -> SNR [1]
        break;
      default:
        printf("noisesim: illegal option %c\n",c);
        argc= 0;
        break;
      }
    }
...
//  float DSNR= pow(10.0,atof(argv[2])/10.0), OSNR= pow(10.0,atof(argv[3])/10.0);	// Argument: SNR [dB] -> SNR [1]
