  ...
  */
  ...
  switch ((*Network).Efilt_CutOff ) { 
    case 7:
      ...
      *SamplingPoint= 51;	// opt: 17.5 ord 6 / elec: 7 ord 2
    break;
    case 14:
...
float Threshold3(GENDRIVE *Carrier, float RZBit1Gauss[], float *OSigmaNoise, float *DSigmaNoise, DeviceSTR *Network, unsigned int transmitters, SuperGaussian *gauss) {
  unsigned long int i;
  unsigned short int Pul, NPulThr= 20;                  // averaged bits to obtain threshold
  ...
  unsigned int Range= 2; 	// even numbers -> effective range SP +- SP/2
  unsigned int Min= SP- Range/2;
  // printf("%i\n",SP); // TEST

  // Sampling
  float Samp0[(Range+ 1)* N0], Samp1[(Range+ 1)* N1];
  ...
  // printf("N0= %i\tN1= %i\n", N0, N1);
  Write1Col("Samp0.dat", Samp0, (Range+ 1)* N0);	// TEST train to file
  Write1Col("Samp1.dat", Samp1, (Range+ 1)* N1);	// TEST train to file

  // Bit 0 statistics
  float sum0= 0, sum_sqr0= 0;
  for(i=0; i<(Range+ 1)* N0; i++) {
    sum0+= Samp0[i];
    Samp0[i]*= Samp0[i];
    sum_sqr0+= Samp0[i];
  }
...
float BitSlots(GENDRIVE *Carrier, SuperGaussian *gauss, DeviceSTR *Network, float OSNR, float *OSigmaNoise, float DSNR, float *DSigmaNoise, unsigned int transmitters, float RZBit1Gauss[], float AuxTemp[]) {	
  ...
  *DSigmaNoise= Pmean_Rxin* (*Network).R* sqrt(1/ DSNR);	// amplitude of noise (Agrawal 4.4.11) [Santiago I.]
  ...
  float Id= Threshold3(Carrier, RZBit1Gauss, OSigmaNoise, DSigmaNoise, Network, transmitters, gauss);
  return Id;
}
