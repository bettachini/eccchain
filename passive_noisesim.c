...
void InputParameters(GENDRIVE *Carrier, SuperGaussian *Gauss, DeviceSTR *Network) {
  ...
  float FAtt1= -2.0 -1.0 -25.0;				// [dB] Tx -> EDFAin 2dB fibre, 1.0dB insertion, 25 dB 128x128 star
  ...
  float FAtt2= -2.0 -1.0;				// [dB] EDFAout -> Rx 2dB fibre, 1.0dB insertion
...
void OpticalFilter(float SignalVector[], unsigned long int SignalVectorSize) {
  ...
  */
  ...
  #define oNZEROS 2
  #define oNPOLES 2
  // #define oGAIN  2.889601535e+02	// BUTTERWORTH CutOff= 12.5GHz, 64 samples
  #define oGAIN  7.145956096e+00	// BUTTERWORTH CutOff= 12.5GHz, 8 samples
...
*/
    ...
    yvo[2] = (xvo[0]+ xvo[2] )+ 2* xvo[1]
      + ( -0.2594951757 * yvo[0]) + (  0.6997380283 * yvo[1]); // BUTTERWORTH CutOff= 12.5GHz, 8 samples
      // +(-0.8406758501 * yvo[0] )+ (1.8268331110* yvo[1] );	// BUTTERWORTH CutOff= 12.5GHz, 64 samples
...
void ElectricalFilter(float SignalVector[], unsigned long int SignalVectorSize, int *SamplingPoint, DeviceSTR *Network) {
  ...
  */
  ...
  /*
  // FILTER ORDER 6 - sample rate 6.4E11
  // #define NZEROSe 6
  // #define NPOLESe 6
  // CutOff= 6GHz order 6
  ...
  */
  ...
  switch ((*Network).Efilt_CutOff ) { 
    case 7:
      // eGAIN= 8.884369142e+02; 	// CutOff= 7 GHz / Sample rate 6.4E11
      eGAIN= 1.858664231e+01; 	// CutOff= 7 GHz / Sample rate 8E10
      *SamplingPoint= 6;	// opt: 17.5 ord 6 / elec: 7 ord 2 / Sample rate 8E10
      //*SamplingPoint= 48;	// opt: 17.5 ord 6 / elec: 7 ord 2 / Sample rate 6.4E11
    break;
    case 14:
      eGAIN= 2.326204969e+02; 	// CutOff= 14 GHz / Sample rate 6.4E11
      *SamplingPoint= 40;	// opt: 12.5 ord 2 / elec: 14 ord 2 / Sample rate 6.4E11
  ...
  for (i=0; i<SignalVectorSize; i++) {
    ...
    //  ( -0.8233495955 * yve[0]) + (  1.8061542062 * yve[1]);  // opt: 17.5 ord 6 / elec: 14 ord 2 / Sample rate 6.4E11
    // ( -0.9073853925 * yve[0]) + (  1.9028831033 * yve[1]);  // opt: 17.5 ord 6 / elec: 7 ord 2 / Sample rate 6.4E11
       + ( -0.4604272288 * yve[0]) + (  1.2452189124 * yve[1]);	// opt= 12.5 ord 2 / elec= 7 ord 2 /sample rate= 8E10
...
*/
  ...
  // Optical filtering
  //OF OpticalFilter(SuperStr, (*Carrier).NPtos_Bit* NPulThr);
  ...
  int SP=6;  //FE
//FE ElectricalFilter(SuperStr, (*Carrier).NPtos_Bit* NPulThr, &SP, Network);
  // Write1Col("SuperStr.dat",SuperStr,(*Carrier).NPtos_Bit* NPulThr);
...
// float BitSlots(GENDRIVE *Carrier, SuperGaussian *Gauss, DeviceSTR *Network, float OSNR, float *OSigmaNoise, float DSNR, float *DSigmaNoise, unsigned int transmitters, float RZBit1Gauss[], float AuxTemp[], float IthFrac) {		// TEST Ith uncomment
  ...
  */
  ...
  for(i=0; i<(*Carrier).NPtos_Bit; i++) Zero[i]= DummyAux;
  //OF OpticalFilter(Zero, (*Carrier).NPtos_Bit);
  DummyAux*= DummyAux;		// Optical: Amplitude -> Power
  DummyAux*= (*Network).M* (*Network).R;	// Optical Power -> Electrical Current 
  for(i=0; i<(*Carrier).NPtos_Bit; i++) Zero[i]= DummyAux;
  int SP;
  //FE ElectricalFilter(Zero, (*Carrier).NPtos_Bit, &SP, Network);
  ...
  //OF OpticalFilter(PmeanVector, 2* (*Carrier).NPtos_Bit);
 ...
 */
  ...
  */
  ...
  // Optical filtering
  //OF OpticalFilter(RZPulsesTrain, (*Carrier).NPtos_Tot);
  ...
  */
  ...
  //FE ElectricalFilter(RZPulsesTrain, (*Carrier).NPtos_Tot, SamplingPoint, Network);
