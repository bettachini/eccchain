  ...
  */
  ...
  while(k<(*Erste).N_Bits){
    if(InBitString[k]==0) for(i=0; i<(*Erste).NPtos_Bit; i++) RZPulsesTrain[k*(*Erste).NPtos_Bit+i][2]=RZBit0[i];
      ...
      for(i=0; i<(*Erste).NPtos_Bit; i++) RZPulsesTrain[k*(*Erste).NPtos_Bit+i][2]=RZBit1Gauss[i];
      senders= InBitString[k];
      if (senders>1) for(l=0; l<(senders-1); l++) for(i=0; i<(*Erste).NPtos_Bit; i++) RZPulsesTrain[k*(*Erste).NPtos_Bit+i][2]+=(RZBit1Gauss[i]-LOff);
    }
    k++;
  }
