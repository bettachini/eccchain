...
void InputParameters(GENDRIVE *Carrier, SuperGaussian *Gauss, DeviceSTR *Network) {
  ...
  // GENDRVIE Carrier
  (*Carrier).NPtos_Bit= pow(2,4);				 		// # points for each bit slot
  (*Carrier).Duty_Cycle= 1.0/3.0;
  (*Carrier).B= 10E9;								// bit rate [bits/s] (10 Gbits/s)
  (*Carrier).Bit_Slot= 1.0/ (*Carrier).B;						// 1/B [s]
  ...
  float FAtt2= -2.0 -1.0;				// [dB] EDFAout -> Rx 2dB fibre, 1.0dB insertion // attenuation passive 128 star = 1x128 splitter
  (*Network).FAtt2= pow(10.0, FAtt2/ 20.0);		// [1] EDFAout -> Rx amplitude
  //float EDFAgain= 27.0;					// [dB] Gain at EDFA <fixed approx>
  float EDFAgain= 0.0;					// [dB] Gain at EDFA <fixed approx>  passive
...
void ElectricalFilter(float SignalVector[], unsigned long int SignalVectorSize, int *SamplingPoint, DeviceSTR *Network, GENDRIVE *Carrier) {
  /* Digital filter designed by mkfilter/mkshape/gencode   A.J. Fisher
 Command line: /www/usr/fisher/helpers/mkfilter -Bu -Lp -o 2 -a 1.5625000000e-02 0.0000000000e+00 -l
  ...
  */
  ...
  switch ((*Network).Efilt_CutOff ) { 
    case 7:
      ...
      *SamplingPoint= floor( (48.0/64.0)* (*Carrier).NPtos_Bit);	// opt: 17.5 ord 6 / elec: 7 ord 2
    break;
    case 14:
      eGAIN= 2.326204969e+02; 	// CutOff= 14 GHz
      *SamplingPoint= floor( (40.0/64.0)* (*Carrier).NPtos_Bit);	// opt: 12.5 ord 2 / elec: 14 ord 2
...
*/
  ...
  // passive for(i=0; i<(*Carrier).NPtos_Bit* NPulThr; i++) SuperStr[i]+= ONoiseVector[i];
  ...
  int SP;
  ElectricalFilter(SuperStr, (*Carrier).NPtos_Bit* NPulThr, &SP, Network, Carrier);
  // Write1Col("SuperStr.dat",SuperStr,(*Carrier).NPtos_Bit* NPulThr);
...
// float BitSlots(GENDRIVE *Carrier, SuperGaussian *Gauss, DeviceSTR *Network, float OSNR, float *OSigmaNoise, float DSNR, float *DSigmaNoise, unsigned int transmitters, float RZBit1Gauss[], float AuxTemp[], float IthFrac) {		// TEST Ith uncomment
  ...
  */
  ...
  for(i=0; i<(*Carrier).NPtos_Bit; i++) Zero[i]= DummyAux;
  int SP;
  ElectricalFilter(Zero, (*Carrier).NPtos_Bit, &SP, Network, Carrier);
  ...
  // passive for(i=0; i< 2* (*Carrier).NPtos_Bit; i++) PmeanVector[i]+= ONoiseVector[i];
 ...
 */
  ...
  // passive for(i=0; i<(*Carrier).NPtos_Tot; i++) RZPulsesTrain[i]+= ONoiseVector[i];	// Noise addition
  ...
  */
  ...
  ElectricalFilter(RZPulsesTrain, (*Carrier).NPtos_Tot, SamplingPoint, Network, Carrier);
